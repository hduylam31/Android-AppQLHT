...
class CalendarService{
      ...
      data.weeks.forEach(week => {
          week.days.forEach(day => {
              day.events.forEach((event)=> {
                  ...
                  const timeString = date.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit', hourCycle: 'h24'}); 
  ...
  static async addUserCalendar(title,textDate,textTime,content){
    try {
      ...
      const textDateProcessed = `${year}-${month.toString().padStart(2, '0')}-${day.toString().padStart(2, '0')}`;
      ...
      const userDoc = await getDoc(userRef);
      if(userDoc.exists()){
        updateDoc(userRef, { "calendar.user": arrayUnion(item)} , {merge: true});
      }else{
  ...
  static async updateUserCalendar(elm){
    ...
    const updatedData = {
        id: elm.c_id, title: elm.title, 
        dateString: textDateProcessed, isNotified: elm.isNotified, 
        timeString: vTime, description: elm.content, isMoodle: elm.c_isMoodle
    }
    ...
    try {
        ...
        const itemIndex = userCalendarData.findIndex(item => item.id === elm.c_id);
        if(itemIndex !== -1){
            const updatedUserCalendarList = [...userCalendarData];
            updatedUserCalendarList[itemIndex] = {...updatedUserCalendarList[itemIndex], ...updatedData};
            await updateDoc(userRef, { "calendar.user": updatedUserCalendarList }, {merge: true});
